function Create-Sheets {
  ...
  foreach ($Sheet in $Exercises) {

    # Set the name of the .tex file
    $Name = $Prefix + $Sheet.Key.ToString("00") + ".tex"

    # Check if the file already exists and skip overwrite is set to false
    if ($Overwrite -eq $false -and (Test-Path $Path$Name -Path-Type Leaf)) {
      continue
    }
    ...
    $Content = @"
      ...
      \Enable{task,homework,exercise,extra,note}
    ...
    foreach ($Item in $Sheet.Value) {
      ...
      $Type = $Item.type ? $Item.type : "exercise"
